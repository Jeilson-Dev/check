...
import 'package:checker/logger.dart';
...
class CodeGuard {
  ...
  Future<List<File>> getFilesFromDirectory(
    ...
    for (var element in entities) {
      if (element is Directory) {
        ...
        files.addAll(await getFilesFromDirectory(
            entities: await element.list().toList()));
      }
      if (element is File) {
        if (isTargetFileType(
            file: element,
            targetFileTypes: targetFileTypes,
            explicitIgnoreSubType: explicitIgnoreSubType)) files.add(element);
      }
    }
  ...
  bool isTargetFileType(
      {required File file,
      required List<String> targetFileTypes,
      List<String> explicitIgnoreSubType = const []}) {
  ...
  bool _hasMatcher({required String line, required List<String> patterns}) {
    ...
    for (var pattern in patterns) {
      ...
      hasMatch =
          line.toLowerCase().contains(RegExp('.*${pattern.toLowerCase()}.*'));
    }
    return hasMatch;
  }
}
